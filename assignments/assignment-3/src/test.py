...
This is done following the documentation by gensim: https://tedboy.github.io/nlps/generated/generated/gensim.models.Word2Vec.most_similar.html 
"""

def expand_query(target_word, topn = 10):

    similar_words = []
    for word, _ in model.most_similar(target_word, topn):
...
in the expanded query (expanded_query).
...
def calculate_percentage(artist_name, expanded_query):
    artist_songs = df[df['artist'] == artist_name]['text'].str.lower()
    total_songs = len(artist_songs)
    songs_with_words = artist_songs.str.contains('|'.join(expanded_query)).sum()
...
def main():
    ...
    expanded_query = expand_query(target_word)
    ...
    percentage = calculate_percentage(artist_name, expanded_query)

    # Print the results
    print(f"{percentage:.2f}% of {artist_name}'s songs contain words related to {target_word}")

    # Save the results as .txt
    filepath = f"out/{artist_name}_{target_word}_results.txt"
    with open(filepath, 'w') as file:
        file.write(f"{percentage:.2f}% of {artist_name}'s songs contain words related to {target_word}")
