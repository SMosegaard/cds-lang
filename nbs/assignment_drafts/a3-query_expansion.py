...
def visualize_multiple_words_from_df(model, df, topn = 10, tracker, outpath):
    all_words = []
    word_labels = []
...
def visualize_query(target_word, similar_words, model, tracker, outpath):

    tracker.start_task("visualize query")
    ...
    plt.title(f"t-SNE visualization of '{target_word}' and similar words")
    plt.savefig(f"{outpath}/t-SNE_{target_word}.png")
    emissions_3_save_results = tracker.stop_task()
    return print("The visualization has been saved to the out folder")
...
def main():
    ...
    visualize_multiple_words_from_df(model, results_df, topn = 10, tracker, "out/t-SNE_nQueries_from_df.png")
